...
if (isset($_SESSION['email'])) {
  ...
  header('Location: login.php'); // redirect or handle the case when the email value is not available in the session
}
  ...
  $searchAddress = mysqli_query($conn, "SELECT Street_address, City, Country FROM Address_of_worker WHERE Worker_ID IN (SELECT ID FROM worker WHERE Email = '$email')");
   ...
   $searchMobile=mysqli_query($conn,"SELECT  Mobile_Number FROM Phone_Number_of_worker WHERE Worker_ID IN (SELECT ID FROM worker WHERE Email = '$email')");
    ...
    $searchedu=mysqli_query($conn,"select ID,discription from education WHERE Worker_ID IN (SELECT ID FROM worker WHERE Email = '$email')");
    $searchexp=mysqli_query($conn,"select ID,Description from experince WHERE Worker_ID IN (SELECT ID FROM worker WHERE Email = '$email')");
    ...
    mysqli_query($conn,"UPDATE worker Set Image_Path='$Dimage' where Email='$email' ");
    ...
    $searchID=mysqli_query($conn,"SELECT ID  from worker where Email = '$email'");
    while ($sidrow= mysqli_fetch_assoc($searchID)) 
    {
        $sid=$sidrow['ID'];
    }
    ...
    if(isset($_POST["addedu"]))
    {
      $edu=$_POST["education"];
      if($edu!=null){
      mysqli_query($conn,"INSERT INTO education(discription,Worker_ID) VALUES('$edu','$sid')");
      header('Location: woker view.php');
      }
    }
    ...
    if(isset($_POST["addexp"]))
    {
      ...
      if($exp!=null){
      mysqli_query($conn,"INSERT INTO experince(Description,Worker_ID) VALUES('$exp','$sid')");
      header('Location: woker view.php');
